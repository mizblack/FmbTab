 ...
 * 
 * <p>
 * The geodetic operations in this class are based on an ellipsoidal model of
 * the Earth, using Vincenty's formulae; this is slow, but very accurate. Be
 * aware, however, that accuracy is compromised in extreme cases, such as
 * nearly-antipodal points.
 * 
 * <p>
 * The user needs to specify the ellipsoid to use for the geodetic calculations;
 * this is done in the constructor. The Ellipsoid enum defines a range of
 * reference ellipsoids. The default is the widely-used WGS-84 standard.
 * 
 * <p>
 * The code in this class was converted from the Fortran implementations
 * developed by the National Geodetic Survey. No license is specified for this
 * code, but I believe that it is public domain, since it was created by an
 ...
 * implementations of Vincenty's formulae,</a> from the National Geodetic Survey
 * </li>
 * <li><a href="http://www.ngs.noaa.gov/PUBS_LIB/inverse.pdf">Vincenty's paper
 * describing his formulae</a>, from NOAA</li>
 * <li><a href="http://www.movable-type.co.uk/scripts/latlong-vincenty.html">JS
 * implementations of Vincenty's inverse formula</a>, from Movable Type Ltd</li>
 * <li><a
 * href="http://www.movable-type.co.uk/scripts/latlong-vincenty-direct.html">JS
 * implementations of Vincenty's direct formula</a>, from Movable Type Ltd</li>
 * </ul>
 * 
 */
public class VincentyDistanceCalculator {
    ...
    public Distance distance(Position p1, Position p2) {
        ...
        double[] ret = gpnhri(ellipsoid.axis, ellipsoid.flat, p1.getLatRads(),
                p1.getLonRads(), p2.getLatRads(), p2.getLonRads());
    ...
    public Position offset(Position p1, Distance distance, Azimuth azimuth) {
        ...
        double[] res = dirct1(p1.getLatRads(), p1.getLonRads(),
                azimuth.getRadians(), distance.getMetres(), ellipsoid.axis,
                ellipsoid.flat);
     ...
     * azimuth and at any distance short of antipodal.
     * 
     * Programmed for the CDC-6600 by lcdr L. Pfeifer, NGS Rockville MD, 20 Feb
     * 1975.
     * 
     * @param glat1
     *            The latitude of the starting point, in radians, positive north.
     * @param glon1
     *            The latitude of the starting point, in radians, positive east.
     * @param azimuth
     *            The azimuth to the desired location, in radians clockwise from north.
     * @param dist
     *            The distance to the desired location, in meters.
     * @param axis
     *            The semi-major axis of the reference ellipsoid, in meters.
     * @param flat
     *            The flattening of the reference ellipsoid.
     * @return An array containing the latitude and longitude of the desired
     *         point, in radians, and the azimuth back from that point to the
     *         starting point, in radians clockwise from north.
     */
    public static double[] dirct1(double glat1, double glon1, double azimuth,
            double dist, double axis, double flat) {
        double r = 1.0 - flat;
     ...
     * rainsford's method with helmert's elliptical terms. Effective in any
     * azimuth and at any distance short of antipodal; from/to stations must not
     * be the geographic pole.
     * 
     * Programmed by Robert (Sid) Safford; released for field use 5 Jul 1975.
     * 
     * @param a
     *            Semi-major axis of reference ellipsoid in meters.
     * @param f
     *            Flattening (0.0033528...).
     * @param p1
     *            Lat station 1, in radians, positive north.
     * @param e1
     *            Lon station 1, in radians, positive east.
     * @param p2
     *            Lat station 2, in radians, positive north.
     * @param e2
     *            Lon station 2, in radians, positive east.
     * @return An array of doubles, containing: the geodetic distance between
     *         the stations, in meters; the azimuth at station 1 to station 2;
     *         and the azimuth at station 2 to station 1. Azimuths are in
     *         radians, clockwise from north, and may not be normalized.
     */
    private static double[] gpnhri(double a, double f, double p1, double e1,
            double p2, double e2) {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            double ao = f - f2 * (1.0 + f + f2) * w / 4.0 + 3.0 * f3
                    * (1.0 + 9.0 * f / 4.0) * t4 / 16.0 - 25.0 * f4 * t6
                    / 128.0;
            double a2 = f2 * (1 + f + f2) * w / 4.0 - f3 * (1 + 9.0 * f / 4.0)
                    * t4 / 4.0 + 75.0 * f4 * t6 / 256.0;
            double a4 = f3 * (1.0 + 9.0 * f / 4.0) * t4 / 32.0 - 15.0 * f4 * t6
                    / 256.0;
        ...
        double b2 = z
                * (-1.0 / 4.0 + z
                        * (1.0 / 16.0 + z * (-15.0 / 512.0 + z * 35.0 / 2048.0)));
        double b4 = z * z
     ...
     * @param flat
     *            The flattening (0.0033528 ... ).
     * @param esq
     *            Eccentricity squared for reference ellipsoid.
     * @param p1
     *            The latitude of station 1.
     * @param p2
     *            The latitude of station 2.
     * @return An array of doubles, containing: the geodesic distance between
     *         the stations, in meters; the azimuth at station 1 to station 2;
    ...
    private static double[] gpnarc(double amax, double flat, double esq,
        ...
        if (!ninety) {
            ...
            s2 = -db * b / 2 + dc * c / 4 - dd * d / 6 + de * e / 8 - df * f
                    / 10;
        }
    ...
    // Floating-point tolerance. Two values closer than this can be considered
    // to be the same.
    ...
    // Points closer to the equator than this are candidates to be consider
    // anti-nodal.
...
class Position {
    public static final Position UNKNOWN = new Position(Double.NaN, Double.NaN,
            true);
    ...
    private Position(double latRadians, double lonRadians, boolean allowNan) {
        if (!allowNan) {
            if (Double.isNaN(latRadians) || Double.isInfinite(latRadians)
                    || Double.isNaN(lonRadians)
                    || Double.isInfinite(lonRadians))
                throw new IllegalArgumentException(
                        "Components of a Position must be finite");
        }
    ...
    public double getCentreDistance() {
        if (Double.isNaN(latitudeR))
            return Double.NaN;
        double ρ = 0.9983271 + 0.0016764 * cos(2 * latitudeR) - 0.0000035
                * cos(4 * latitudeR);
        return ρ;
    }
...
class Angle {
    ...
    public static void formatDegMin(double angle, char pos, char neg,
            StringBuilder sb) {
        if (sb.length() != 12)
            sb.setLength(12);
        if (angle < 0) {
    ...
    public static String formatDegMinSec(double angle, char posSign,
            char negSign) {
